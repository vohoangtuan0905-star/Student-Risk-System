...
const parseExcelBuffer = (buffer) => {
  ...
  if (!sheetName) throw new Error('Kh�ng t�m th?y sheet trong file Excel');
...
exports.previewImportLecturers = async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ message: 'Vui l�ng ch?n file Excel' });
...
exports.importLecturers = async (req, res) => {
  try {
    ...
    if (!mapping.full_name || !mapping.email) {
      return res.status(400).json({ message: 'Thi?u mapping cho full_name v� email' });
    }
    ...
    for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
      ...
      if (!fullName || !email) {
        errors.push({ row: rowIndex + 1, message: 'Thi?u H? t�n ho?c Email' });
        continue;
      }
      ...
      if (deptCode) {
        ...
        if (!deptId) {
          errors.push({ row: rowIndex + 1, message: `Kh�ng t�m th?y Khoa c� m�: ${deptCode}` });
          continue;
        }
      }
    ...
    res.json({ message: 'Import Gi?ng vi�n ho�n t?t', createdCount, updatedCount, failedCount: errors.length, errors });
  } catch (error) {
    console.error('importLecturers error:', error);
    res.status(500).json({ message: 'L?i server khi import Gi?ng vi�n' });
  }
};
